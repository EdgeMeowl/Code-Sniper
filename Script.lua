...
local TweenService      = cloneref(game:GetService("TweenService"))  -- <-- ADDED
...
-- ===== SECOND FILE HELPERS (UI scanning, submission) =====
...
-- ===== UI COLORS =====
local COLORS = {
    Window = Color3.fromRGB(13, 13, 16),
    Row = Color3.fromRGB(22, 22, 28),
    Control = Color3.fromRGB(31, 31, 39),
    Black = Color3.fromRGB(0, 0, 0),
    Log = Color3.fromRGB(17, 17, 22),
    Border = Color3.fromRGB(47, 47, 58),
    White = Color3.fromRGB(245, 245, 245),
    Text = Color3.fromRGB(205, 205, 215),
    Dim = Color3.fromRGB(122, 122, 138),
    Accent = Color3.fromRGB(0, 170, 255),
    AccentSoft = Color3.fromRGB(0, 120, 190),
...
-- ===== CLEANUP OLD GUIS =====
pcall(function()
    for _, name in ipairs({"ACECodeSniperUI", "AutoTypeCodesUI", "ACEPaste", "EdgeMeowlSniper"}) do
...
-- ZERK STYLE BORDER (spinning outline)
local BorderFrame, BorderScale, BorderGradient

BorderFrame = Instance.new("Frame")
BorderFrame.Name = "BorderFrame"
BorderFrame.Size = UDim2.fromOffset(WINDOW_W + 4, WINDOW_H + 4)
BorderFrame.AnchorPoint = Vector2.new(1, 0)
BorderFrame.Position = UDim2.new(1, -6, 0, 6)
BorderFrame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
BorderFrame.BorderSizePixel = 0
BorderFrame.ZIndex = 0
BorderFrame.Parent = GUI
addCorner(BorderFrame, 14)
...
BorderScale = Instance.new("UIScale")
BorderScale.Name = "BorderScale"
BorderScale.Scale = InterfaceScale.Scale
BorderScale.Parent = BorderFrame

-- Spin the border gradient using TweenService
task.spawn(function()
    while BorderGradient and BorderGradient.Parent do
        local t = TweenService:Create(BorderGradient, TweenInfo.new(3.5, Enum.EasingStyle.Linear), {Rotation = 360})
        t:Play()
        t.Completed:Wait()
...
local function updateInterfaceScale()
    ...
    local fitScale = math.min((viewport.X - 16) / WINDOW_W, (viewport.Y - 16) / WINDOW_H)
    if UserInputService.TouchEnabled then
        InterfaceScale.Scale = math.max(0.75, math.min(1.0, fitScale))
    else
        InterfaceScale.Scale = 0.92
    end
    if BorderScale then BorderScale.Scale = InterfaceScale.Scale end
end
...
Header.Name = "Header"
Header.Size = UDim2.new(1, 0, 0, UserInputService.TouchEnabled and 74 or 64)
...
makeLabel(Header, "Title", "EDGEMEOWL SNIPER", UDim2.fromOffset(180, 25), UDim2.fromOffset(15, UserInputService.TouchEnabled and 14 or 10), UserInputService.TouchEnabled and 18 or 15, COLORS.White, Enum.Font.GothamBold)
makeLabel(Header, "Subtitle", "promo code auto-redeem", UDim2.fromOffset(180, 15), UDim2.fromOffset(15, UserInputService.TouchEnabled and 42 or 34), UserInputService.TouchEnabled and 11 or 9, COLORS.Dim, Enum.Font.Gotham)

local HeaderDivider = Instance.new("Frame")
HeaderDivider.Name = "HeaderDivider"
HeaderDivider.Size = UDim2.new(1, -30, 0, 2)
HeaderDivider.Position = UDim2.fromOffset(15, UserInputService.TouchEnabled and 69 or 59)
HeaderDivider.BackgroundColor3 = COLORS.Accent
HeaderDivider.BorderSizePixel = 0
HeaderDivider.Parent = Header

-- Minimize Button
local MinimizeButton, minimized

local function buildHeaderButtons()
    MinimizeButton = Instance.new("TextButton")
    MinimizeButton.Name = "MinimizeButton"
    MinimizeButton.Size = UDim2.fromOffset(UserInputService.TouchEnabled and 30 or 24, UserInputService.TouchEnabled and 30 or 24)
    MinimizeButton.Position = UDim2.new(1, UserInputService.TouchEnabled and -40 or -34, 0, UserInputService.TouchEnabled and 8 or 9)
    MinimizeButton.BackgroundColor3 = COLORS.Black
    MinimizeButton.AutoButtonColor = false
    MinimizeButton.Text = "–"
    MinimizeButton.TextColor3 = COLORS.Accent
    MinimizeButton.TextSize = UserInputService.TouchEnabled and 14 or 12
    MinimizeButton.Font = Enum.Font.GothamBold
    MinimizeButton.ZIndex = 6
    MinimizeButton.Parent = Header
    addCorner(MinimizeButton, 4)
    addStroke(MinimizeButton, COLORS.Border, 1, 0)
    ...
    local function toggleMinimize()
        ...
        TweenService:Create(Window, TweenInfo.new(0.2), {
            Size = UDim2.fromOffset(WINDOW_W, minimized and 32 or WINDOW_H)
        }):Play()
        if BorderFrame then
            TweenService:Create(BorderFrame, TweenInfo.new(0.2), {
                Size = UDim2.fromOffset(WINDOW_W + 4, (minimized and 32 or WINDOW_H) + 4)
            }):Play()
        end
    end
    ...
    if UserInputService.TouchEnabled then
        local MinimizeTouchTarget = Instance.new("TextButton")
        MinimizeTouchTarget.Name = "MinimizeTouchTarget"
        MinimizeTouchTarget.Size = UDim2.fromOffset(56, 56)
        MinimizeTouchTarget.Position = UDim2.new(1, -60, 0, 2)
        MinimizeTouchTarget.BackgroundTransparency = 1
        MinimizeTouchTarget.Text = ""
        MinimizeTouchTarget.AutoButtonColor = false
        MinimizeTouchTarget.ZIndex = 10
        MinimizeTouchTarget.Parent = Header
        MinimizeTouchTarget.Activated:Connect(toggleMinimize)
    end
end
buildHeaderButtons()

-- Tab Bar
local TAB_Y = UserInputService.TouchEnabled and 76 or 66
local CONTENT_Y = UserInputService.TouchEnabled and 114 or 104

local TabBar = Instance.new("Frame")
TabBar.Name = "TabBar"
TabBar.Size = UDim2.new(1, -30, 0, 32)
TabBar.Position = UDim2.fromOffset(15, TAB_Y)
...
MainTab.Size = UDim2.new(1, -30, 0, WINDOW_H - CONTENT_Y - 36)
MainTab.Position = UDim2.fromOffset(15, CONTENT_Y)
...
LogTab.Name = "LogTab"
LogTab.Size = MainTab.Size
LogTab.Position = MainTab.Position
LogTab.BackgroundTransparency = 1
LogTab.ZIndex = 3
LogTab.Visible = false
LogTab.Parent = Window
...
local function makeTab(name, xOffset)
    ...
    button.Text = name
    button.TextColor3 = name == currentTab and COLORS.White or COLORS.Dim
    button.TextSize = UserInputService.TouchEnabled and 13 or 11
    button.Font = name == currentTab and Enum.Font.GothamBold or Enum.Font.Gotham
...
function setTab(name)
    currentTab = name
    for _, t in ipairs({"MAIN", "LOG"}) do
        local active = t == name
        tabButtons[t].TextColor3 = active and COLORS.White or COLORS.Dim
        tabButtons[t].Font = active and Enum.Font.GothamBold or Enum.Font.Gotham
        tabIndicators[t].Visible = active
    end
...
-- Sniper Toggle (big button)
local AutoWriteButton = Instance.new("TextButton")
AutoWriteButton.Name = "AutoWrite"
AutoWriteButton.Size = UDim2.new(1, -30, 0, UserInputService.TouchEnabled and 56 or 48)
AutoWriteButton.Position = UDim2.fromOffset(15, 4)
AutoWriteButton.TextSize = UserInputService.TouchEnabled and 16 or 14
AutoWriteButton.Font = Enum.Font.GothamBold
AutoWriteButton.BackgroundColor3 = COLORS.Black
AutoWriteButton.BorderSizePixel = 0
AutoWriteButton.AutoButtonColor = false
AutoWriteButton.Active = true
AutoWriteButton.ZIndex = 6
AutoWriteButton.Parent = MainTab
addCorner(AutoWriteButton, 6)
addStroke(AutoWriteButton, COLORS.Border, 1.2, 0)

local function renderAutoWriteButton()
    if autoWriteEnabled then
        AutoWriteButton.Text = "SNIPER ACTIVE"
        AutoWriteButton.BackgroundColor3 = COLORS.Black
        AutoWriteButton.TextColor3 = COLORS.White
    else
        AutoWriteButton.Text = "SNIPER OFF"
        AutoWriteButton.BackgroundColor3 = COLORS.Black
        AutoWriteButton.TextColor3 = COLORS.Dim
    end
end
...
-- ===== SETTINGS ROWS =====
local ROW_H = UserInputService.TouchEnabled and 46 or 40
local LABEL_SIZE = UserInputService.TouchEnabled and 13 or 11
local ROWS_Y = UserInputService.TouchEnabled and 68 or 60
...
local function makeRow(name, yPos)
    ...
    line.Position = UDim2.fromOffset(0, -1)
...
local function makeTogglePill(parent, startOn, textSize, onChanged)
    local button = Instance.new("TextButton")
    button.Size = UDim2.fromOffset(UserInputService.TouchEnabled and 76 or 60, UserInputService.TouchEnabled and 26 or 20)
    button.Position = UDim2.new(1, UserInputService.TouchEnabled and -90 or -74, 0, 0)
    button.BackgroundColor3 = COLORS.Black
    button.BorderSizePixel = 0
    button.TextColor3 = startOn and COLORS.Window or COLORS.Dim
    button.Text = startOn and "ON" or "OFF"
    button.TextSize = textSize or 10
    button.Font = Enum.Font.GothamBold
    button.Parent = parent
    addCorner(button, 4)
    addStroke(button, COLORS.Border, 1, 0)
...
local function makeStateToggle(parent, consoleName, startOn, onChanged)
    local button = makeTogglePill(parent, startOn, UserInputService.TouchEnabled and 12 or 10, function(state)
        featureStates[consoleName] = state
        if autoWriteEnabled then appendConsoleStatus(consoleName, state) end
        if onChanged then onChanged(state) end
    end)
    featureStates[consoleName] = startOn and true or false
    return button
end

local function centerInRow(row, control)
    control.Position = UDim2.new(1, -(control.Size.X.Offset + 12), 0.5, -control.Size.Y.Offset / 2)
end

local AutoRow = makeRow("AutoSubmit", ROWS_Y)
makeLabel(AutoRow, "Title", "Auto submit", UDim2.new(1, -110, 1, 0), UDim2.fromOffset(4, 0), LABEL_SIZE, COLORS.White, Enum.Font.GothamSemibold)
local AutoPill = makeStateToggle(AutoRow, "Auto submit", _autoAccept, function(state)
    _autoAccept = state
    savedConfig.autoSubmit = state
    saveConfig()
end)
centerInRow(AutoRow, AutoPill)

local DelayRow = makeRow("SubmitAfter", ROWS_Y + ROW_H)
makeLabel(DelayRow, "Title", "Submit after msgs", UDim2.new(1, -130, 1, 0), UDim2.fromOffset(4, 0), LABEL_SIZE, COLORS.White, Enum.Font.GothamSemibold)

local CounterShell = Instance.new("Frame")
CounterShell.Name = "Counter"
CounterShell.Size = UDim2.fromOffset(UserInputService.TouchEnabled and 116 or 98, UserInputService.TouchEnabled and 36 or 30)
CounterShell.BackgroundColor3 = COLORS.Control
CounterShell.BorderSizePixel = 0
CounterShell.Parent = DelayRow
addCorner(CounterShell, 6)
addStroke(CounterShell, COLORS.Border, 1, 0)
centerInRow(DelayRow, CounterShell)

local Minus = Instance.new("TextButton")
Minus.Name = "Minus"
Minus.Size = UDim2.fromOffset(UserInputService.TouchEnabled and 30 or 24, UserInputService.TouchEnabled and 30 or 24)
Minus.Position = UDim2.fromOffset(3, 3)
Minus.BackgroundColor3 = COLORS.Black
Minus.BorderSizePixel = 0
Minus.AutoButtonColor = false
Minus.Active = true
Minus.Text = "-"
Minus.TextSize = UserInputService.TouchEnabled and 18 or 16
Minus.TextColor3 = COLORS.Accent
Minus.Font = Enum.Font.GothamBold
Minus.Parent = CounterShell
addCorner(Minus, 4)

local Count = makeLabel(CounterShell, "Count", tostring(_submitAfter), UDim2.fromOffset(UserInputService.TouchEnabled and 44 or 28, UserInputService.TouchEnabled and 30 or 24), UDim2.fromOffset(UserInputService.TouchEnabled and 38 or 35, 3), UserInputService.TouchEnabled and 18 or 17, COLORS.White, Enum.Font.GothamBold)
Count.TextXAlignment = Enum.TextXAlignment.Center

local Plus = Instance.new("TextButton")
Plus.Name = "Plus"
Plus.Size = UDim2.fromOffset(UserInputService.TouchEnabled and 30 or 24, UserInputService.TouchEnabled and 30 or 24)
Plus.Position = UDim2.fromOffset(UserInputService.TouchEnabled and 83 or 71, 3)
Plus.BackgroundColor3 = COLORS.Black
Plus.BorderSizePixel = 0
Plus.AutoButtonColor = false
Plus.Active = true
Plus.Text = "+"
Plus.TextSize = UserInputService.TouchEnabled and 18 or 16
Plus.TextColor3 = COLORS.Accent
...
local RetypeRow = makeRow("RetypeInvalid", ROWS_Y + ROW_H * 2)
makeLabel(RetypeRow, "Title", "Retype invalid", UDim2.new(1, -110, 1, 0), UDim2.fromOffset(4, 0), LABEL_SIZE, COLORS.White, Enum.Font.GothamSemibold)
local RetypePill = makeStateToggle(RetypeRow, "Retype invalid", _retypeInvalid, function(state)
    _retypeInvalid = state
    savedConfig.retypeInvalid = state
    saveConfig()
end)
centerInRow(RetypeRow, RetypePill)

local RiddleRow = makeRow("AIRiddles", ROWS_Y + ROW_H * 3)
makeLabel(RiddleRow, "Title", "Riddle solver", UDim2.new(1, -110, 1, 0), UDim2.fromOffset(4, 0), LABEL_SIZE, COLORS.White, Enum.Font.GothamSemibold)
local RiddlePill = makeStateToggle(RiddleRow, "Riddle solver", _riddleSolver, function(state)
    _riddleSolver = state
    savedConfig.riddleSolver = state
    saveConfig()
end)
centerInRow(RiddleRow, RiddlePill)

-- Console (Log tab)
local LOG_Y = UserInputService.TouchEnabled and 10 or 8
local LOG_H = (WINDOW_H - 36) - CONTENT_Y - LOG_Y - (UserInputService.TouchEnabled and 8 or 4)

Console = Instance.new("ScrollingFrame")
Console.Name = "Console"
Console.Size = UDim2.new(1, 0, 0, LOG_H)
Console.Position = UDim2.fromOffset(0, LOG_Y)
...
Console.ScrollBarImageColor3 = COLORS.Accent
Console.ZIndex = 4
Console.Parent = LogTab
addCorner(Console, 6)
addStroke(Console, COLORS.Border, 1, 0)

ConsoleOutput = Instance.new("TextLabel")
ConsoleOutput.Name = "ConsoleOutput"
ConsoleOutput.Size = UDim2.new(1, -18, 0, 108)
...
ConsoleOutput.TextSize = UserInputService.TouchEnabled and 15 or 14
...
local DiscordFooter = makeLabel(Window, "DiscordFooter", "https://discord.gg/fFS4Uu5NRs", UDim2.fromOffset(140, 18), UDim2.new(0.5, -70, 0, UserInputService.TouchEnabled and 436 or 428), UserInputService.TouchEnabled and 12 or 10, COLORS.Dim, Enum.Font.GothamBold)
DiscordFooter.TextXAlignment = Enum.TextXAlignment.Center
DiscordFooter.BackgroundTransparency = 1
DiscordFooter.ZIndex = 4
...
do
    ...
    local DRAG_THRESHOLD = UserInputService.TouchEnabled and 12 or 3

    local function isOverHeaderControl(position)
        local controls = {}
        if AutoWriteButton and AutoWriteButton.Parent then
            table.insert(controls, { AutoWriteButton.AbsolutePosition, AutoWriteButton.AbsoluteSize })
        end
        if MinimizeButton and MinimizeButton.Parent then
            table.insert(controls, { MinimizeButton.AbsolutePosition, MinimizeButton.AbsoluteSize })
        end
        for _, c in ipairs(controls) do
            local btnPos, btnSize = c[1], c[2]
            if position.X >= (btnPos.X - 10)
    ...
    UserInputService.InputChanged:Connect(function(input)
        ...
        if BorderFrame then
            BorderFrame.Position = UDim2.new(
                startPosition.X.Scale,
                startPosition.X.Offset + delta.X + 2,
                startPosition.Y.Scale,
                startPosition.Y.Offset + delta.Y - 2
            )
        end
    end)
end
    ...
    while current do
        if current:IsA("GuiObject") then
            table.insert(_boxVisibilityConns, current:GetPropertyChangedSignal("Visible"):Connect(function()
                if not isVisibleChain(box) then resetPasteCounter(); clearBoxWatchers() end
            end))
...
local function onAceAnnouncement(...)
    ...
    -- Riddle solver (if enabled and text contains spaces)
    ...
    -- Skip multi-word announcements that aren't riddles
